    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200/80 transition-colors">
      <div className="app-container">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          {/* Left: Brand Logo + AI PLATFORM badge */}
          <div 
            className="flex items-center space-x-3 cursor-pointer select-none group shrink-0" 
            onClick={() => handleNavClick('dashboard')}
          >
            <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-indigo-50 border border-indigo-100 text-[#635BFF] shadow-sm group-hover:scale-105 transition-transform">
          ...
          {/* Center: Persistent Navigation Bar */}
          <nav className="hidden md:flex items-center space-x-1 bg-slate-100/70 p-1.5 rounded-2xl border border-slate-200/80">
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-150 ${
                    isActive
                      ? 'bg-white text-[#635BFF] shadow-sm border border-slate-200/90'
          ...
          {/* Right: YOLO26-Pose Active Status + User Profile + Logout */}
          <div className="hidden lg:flex items-center space-x-3 shrink-0">
            {/* Status Indicator */}
            <div className="flex items-center space-x-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200/80 text-emerald-700 text-xs font-semibold">
            ...
            {user && (
              <div className="flex items-center space-x-2 pl-2 border-l border-slate-200">
                <div className="flex items-center space-x-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200/80 text-slate-800">
                ...
                <button
                  onClick={logout}
                  className="p-2 rounded-xl bg-white border border-slate-200 text-slate-500 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition-colors shadow-xs"
                  title="Sign Out"
                >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 bg-white px-4 pt-3 pb-5 space-y-2 shadow-lg">
          <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-semibold mb-3">
